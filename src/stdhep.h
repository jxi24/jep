...
enum select_t { stdhep_all, stdhep_jets };

template<size_t kMaxGenParticle=2000, size_t kMaxEvent=1>
struct stdhep {
  ...
  stdhep() { }
  stdhep(TTree* tree, select_t branches=stdhep_all) { SetTree(tree,branches); }

  void SetTree(TTree* tree, select_t branches=stdhep_all) {
    this->tree = tree;
